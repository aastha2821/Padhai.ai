...
import { useMascot } from "./mascot-overlay"
...
export default function FlappyBird() {
  ...
  useEffect(() => {
    ...
    const placePipes = () => {
      ...
      // Get questions from localStorage directly to ensure latest
      const storedQ = localStorage.getItem("kidQuizQuestions")
      const currentQuestions = storedQ ? JSON.parse(storedQ) : QUESTIONS
      const pipeQuestion = currentQuestions[Math.floor(Math.random() * currentQuestions.length)]
    ...
    const update = () => {
      ...
      if (currentTime - state.lastPipeTime > 2000 && !state.questionActive) {
        placePipes()
      ...
      // Only update bird physics if question is not active
      ...
      if (state.bird.y > state.boardHeight && !state.questionActive) {
        state.gameOver = true
        setGameOver(true)
      }

      // Draw pipes and handle collisions only when question is not active
      if (!state.questionActive) {
        ...
        // Still draw pipes while question is active but don't move them
        for (let i = 0; i < state.pipeArray.length; i++) {
          const pipe = state.pipeArray[i]
          ctx.drawImage(pipe.img, pipe.x, pipe.y, pipe.width, pipe.height)
        }
      ...
      // If question is active, show pause indicator
      if (state.questionActive) {
        ...
        ctx.fillText("PAUSED - Answer the question", state.boardWidth / 2, state.boardHeight / 2)
        ctx.textAlign = "left"
      }
  ...
  const handleAnswer = (selectedIndex: number) => {
    ...
    if (correctAnswer === userAnswer) {
      ...
      // Update mascot - happy if score is good
      mascot.setEmotion(newScore > 5 ? "Happy" : "Normal")
      mascot.setMessage("Awesome! Keep it up! 🎉")
      ...
      // Update mascot - sad when wrong
      mascot.setEmotion("Sad")
      mascot.setMessage("Don't worry, try again! 💪")
