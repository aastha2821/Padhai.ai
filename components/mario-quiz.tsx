          ...
          if (!enemy.defeated) {
            ...
            if (isOverlapping) {
              ...
              if (isFallingOnTop) {
                ...
                // Show random question from available questions - read from localStorage directly
                const storedQ = localStorage.getItem("kidQuizQuestions")
                const currentQuestions = storedQ ? JSON.parse(storedQ) : sampleQuestions
                setCurrentQuestion(currentQuestions[Math.floor(Math.random() * currentQuestions.length)])
