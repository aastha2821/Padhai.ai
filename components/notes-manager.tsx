...
export function NotesManager({
  ...
  const handleGenerateNotes = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      console.log('[NOTES MANAGER] Generating notes for:', topic, 'count:', count);
      ...
      console.log('[NOTES MANAGER] Response status:', res.status);
      const data = await res.json();
      console.log('[NOTES MANAGER] Response data:', data);
      ...
      if (data.status === "success" && data.notes) {
        ...
        // Refresh saved files
        loadSavedFiles();
        alert(`Successfully generated ${data.notes.length} notes!`);
      } else {
        const errorMsg = data.message || data.error || 'Unknown error';
        console.error('[NOTES MANAGER] Error:', errorMsg);
        alert(`Error: ${errorMsg}`);
      }
    } catch (error) {
      console.error('[NOTES MANAGER] Exception:', error);
      alert(`Failed to generate notes: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
      setLoading(false);
    }
  };
